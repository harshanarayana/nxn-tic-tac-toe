...
"""
Taken from : https://github.com/deadpixi/contracts
...
def build_call(func, *args, **kwargs):
    ...
    for name, value in actual.items():
        if value is nonce:
            raise TypeError("%s missing required positional argument: '%s'" % (
            func.__name__, name))
...
def condition(description, predicate, precondition=False, postcondition=False,
              instance=False):
...
def transform(transformer):
    ...
    assert arg_count(
        transformer) == 1, "transformers can only take a single argument"
...
def invariant(desc, predicate):
    ...
    def invariant(c):
        def check(name, func):
            exceptions = (
            "__getitem__", "__setitem__", "__lt__", "__le__", "__eq__",
            "__ne__", "__gt__", "__ge__", "__init__")

            if name.startswith("__") and name.endswith(
                    "__") and name not in exceptions:
                return False
        ...
        for name, value in [(name, getattr(c, name)) for name in dir(c)]:
            if check(name, value):
                setattr(InvariantContractor, name,
                        condition(desc, predicate, name != "__init__", True,
                                  True)(value))
